'''
This is an example code for deep robust regression (under covariate shift). 
...
def predict_regression(weight, Myy, Myx, output, mean0, var0, device):
    ...
    Myy = torch.tensor(Myy).to(device)
    Myx = torch.tensor(Myx).to(device)

    meanY = (1.0/(2.0*weight*Myy+(1/var0)))*(-2.0*(torch.addcmul(torch.zeros(1).to(device), x_1.matmul(Myx).t(), weight))+(1/var0)*mean0)
...
def train_regression(args, model, device, train_loader, optimizer, epoch, Myy, Myx, mean0, var0):
    ...
    for batch_idx, (data, target, weight) in enumerate(train_loader):
        ...
        output_last = regression_gradient.apply(output, torch.tensor(Myx[0:-1]), torch.reshape(target, (bs, 1)), torch.reshape(meanY, (bs, 1)))
    ...
    print('gradient yy:', np.linalg.norm(np.mean(grad_yy)))
    print('gradient yx:', np.linalg.norm(np.mean(grad_yx, 1)))
...
class Mydata(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        features, target = self.raw_data[index][0:-1], self.raw_data[index][-1]
...
def main():
    ...
    weighted_train = WEIGHT_DATA_SET(train_data, weight_st)
    m_test = len(test_data)
    
    # for covariate shift, plug in densities for target data
    weight_st = np.ones(m_test)
    weighted_test = WEIGHT_DATA_SET(test_data, weight_st)
